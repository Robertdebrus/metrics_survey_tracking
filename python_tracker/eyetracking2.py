...
#    try:
    x0, y0, *data = pag.locateOnScreen('upper_left.png', region=(
        screen_coords['xmin'], screen_coords['ymin'], screen_coords['xmax'] * 0.5, screen_coords['ymax'] * 0.5))
    x, y, w, h = pag.locateOnScreen('bottom_right.png', region=(
        screen_coords['xmin'] * 0.5, screen_coords['ymin'] * 0.5, screen_coords['xmax'], screen_coords['ymax']))
    code_coords['xmin'] = x0
    code_coords['ymin'] = y0
...
class GazeDataController(object):
    @cherrypy.expose
    # turn HTTP payload into an object; also checking the Content-Type header
...
def save_data_to_file(data, location):
    fn = '../eye_data/' + location + '/' + \
        str(UID) + "_" + str(FID) + "_" + str(data['time']) + '.txt'
    print(fn)
    with open(fn, 'w+') as f:
        # copy the data before saving it, since json.dump iterates over the data, 
...
#    try:
    ...
    print("Success! Eyetracking enabled on Java Survey at",
